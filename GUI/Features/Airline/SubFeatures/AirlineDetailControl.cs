...
namespace GUI.Features.Airline.SubFeatures {
    public class AirlineDetailControl : UserControl {
        private Label vCode, vName, vCountry, vAircrafts;

        public AirlineDetailControl() { InitializeComponent(); BuildLayout(); }
        ...
        private void BuildLayout() {
            var title = new Label { Text = "🏢 Chi tiết hãng hàng không", AutoSize = true, Font = new Font("Segoe UI", 20, FontStyle.Bold), Padding = new Padding(24, 20, 24, 0), Dock = DockStyle.Top };
            ...
            grid.RowStyles.Add(new RowStyle(SizeType.AutoSize)); grid.Controls.Add(Key("Số máy bay:"), 0, r); vAircrafts = Val("vAircrafts"); grid.Controls.Add(vAircrafts, 1, r++);
        ...
        public void LoadAirlineInfo(string code, string name, string country, string aircrafts) {
            vCode.Text = code ?? "";
            vName.Text = name ?? "";
            vCountry.Text = country ?? "";
            vAircrafts.Text = aircrafts ?? "0";
        }
    }
}
